fn main() {
    ...
    if target == "wasm32-wasip2" {
        println!("cargo:rustc-link-lib=static=tree_sitter_r");
        println!("cargo:rustc-link-search=native={}/lib", manifest);
        println!("cargo:rerun-if-changed=lib/libtree_sitter_r.a");
        return;
    }
    ...
    cfg.file("src/parser.c").file("src/scanner.c").compile("tree_sitter_r");
